...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = superiorAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date){
    // time of superior birth 2014-04-18 10:49:53 (1397818193)
    var superiorBirthTime = 1442753710;
    // avg seconds per block in v1
    var secondsPerBlockV1 = 60;
    // time of v2 fork 2016-03-23 15:57:38 (1458748658)
    var forkTime = 1442763710;
    // v2 fork block
    var forkBlock = 3656;
    ...
    var secondsPerBlock;
    // before superior's birth
    if (requestedTime < superiorBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < superiorBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > superiorBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - superiorBirthTime)/secondsPerBlockV1);
