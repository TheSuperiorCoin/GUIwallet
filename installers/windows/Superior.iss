; Superior Boron Butterfly GUI Wallet Installer for Windows
...
LicenseFile=LICENSE
AppPublisher=The Superior Developer Community
AppPublisherURL=https://getsuperior.org
...
; Background info:
; This installer indeed asks for admin rights so the Superior files can be copied to a place where they have at least
...
; As of version 0.14.0.0, the Superior .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Superior.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Superior GUI wallet exe and guide
Source: "bin\superior-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\superior-GUI-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "bin\superior-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "bin\superior-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\superior-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\superior-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\superior-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\superior-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\superior-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\superior-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\superior-project\superior-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Superior default location
...
begin
  Result := BlockChainDir('') + '\bitsuperior.log';
...
var s: String;
begin
  s := ExpandConstant('{app}\superior-wallet-gui.log');
  if Pos(' ', s) > 0 then begin
    // Quotes needed for filename with blanks
    s := '"' + s + '"';
  end;
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "superior-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Superior daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\superiord.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\superior-daemon.bat'), s, false);
  end;
end;
...
[Icons]
; Icons in the "Superior GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\superior-wallet-gui.exe"; Parameters: {code:WalletFlags}
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\superior-GUI-guide.pdf"; IconFilename: "{app}\superior-wallet-gui.exe"
...
Name: "{group}\Utilities\Superior Daemon"; Filename: "{app}\superiord.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\superior-wallet-cli.exe"; WorkingDir: "{userdocs}\Superior\wallets"
...
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\superior-wallet-gui.exe"; Parameters: {code:WalletFlags}; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\superior-project\superior-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};
