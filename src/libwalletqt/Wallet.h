...
#include "wallet/api/wallet2_api.h" // we need to have an access to the Superior::Wallet::Status enum here;
...
public:
    ...
    enum Status {
        Status_Ok       = Superior::Wallet::Status_Ok,
        Status_Error    = Superior::Wallet::Status_Error,
        Status_Critical = Superior::Wallet::Status_Critical
    };

    Q_ENUM(Status)

    enum ConnectionStatus {
        ConnectionStatus_Connected       = Superior::Wallet::ConnectionStatus_Connected,
        ConnectionStatus_Disconnected    = Superior::Wallet::ConnectionStatus_Disconnected,
        ConnectionStatus_WrongVersion    = Superior::Wallet::ConnectionStatus_WrongVersion
    };
...
private:
    ...
    Wallet(Superior::Wallet *w, QObject * parent = 0);
    ~Wallet();
private:
    friend class WalletManager;
    friend class WalletListenerImpl;
    //! libwallet's
    Superior::Wallet * m_walletImpl;
