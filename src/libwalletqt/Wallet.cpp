...
class WalletListenerImpl : public  Superior::WalletListener
{
public:
...
{
    QFuture<Superior::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Superior::Wallet::connected);
    QFutureWatcher<Superior::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Superior::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Superior::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Superior::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Superior::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Superior::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Superior::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Superior::optional<uint64_t>(), mixin_count,
                static_cast<Superior::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Superior::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Superior::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Superior::Wallet::genPaymentId());
}
...
Wallet::Wallet(Superior::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Superior::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
