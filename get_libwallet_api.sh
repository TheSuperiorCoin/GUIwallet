...
git -C $SUPERIOR_DIR config user.email "gui@superior-coin.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging TheSuperiorCoin push request #$PR"
    # fetch pull request and merge
    git -C $SUPERIOR_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $SUPERIOR_DIR merge --quiet PR-$PR  -m "Merge TheSuperiorCoin PR #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "TheSuperiorCoin/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_SUPERIOR_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_SUPERIOR_VERSION) is already built. Remove TheSuperiorCoin/lib/libwallet_merged.a to force rebuild"
    fi
fi
...
echo "cleaning up existing TheSuperiorCoin build dir, libs and includes"
