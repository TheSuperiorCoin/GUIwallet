...
# Build GUI
        ...
        cd android/docker
        docker build -t superior-gui-android .
        docker create -it --name superior-gui-android superior-gui-android bash

# Get the apk

        docker cp superior-gui-android:/opt/android/superior-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/superior-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
